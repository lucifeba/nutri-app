...
import { Calendar, MessageCircle, RefreshCw, ChevronLeft, ChevronRight } from "lucide-react"
...
const mockMenu: MenuDay[] = [
    { day: "Monday", meals: { breakfast: { name: "Oatmeal with Berries", calories: 350, protein: 12 }, lunch: { name: "Grilled Chicken Salad", calories: 450, protein: 40 }, dinner: { name: "Salmon with Asparagus", calories: 500, protein: 35 }, snack: { name: "Greek Yogurt", calories: 150, protein: 15 } } },
    { day: "Tuesday", meals: { breakfast: { name: "Avocado Toast", calories: 400, protein: 10 }, lunch: { name: "Turkey Wrap", calories: 420, protein: 30 }, dinner: { name: "Lean Steak with Veggies", calories: 600, protein: 50 }, snack: { name: "Apple & Almonds", calories: 200, protein: 5 } } },
    // ... more days
]
...
export default function PatientDashboard() {
    ...
    const handleSwap = (mealType: string) => {
        alert(`Swapping ${mealType}... Opening exchange list.`)
    }
    ...
    return (
        <div className="min-h-screen bg-background pb-20 md:pb-0">
            <header className="bg-card p-4 shadow-sm border-b border-border flex justify-between items-center sticky top-0 z-10">
                <div className="font-bold text-xl">My<span className="text-primary">Plan</span></div>
                <div className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center text-primary text-sm font-bold">JP</div>
            </header>
            ...
            <main className="max-w-md mx-auto p-4 space-y-6">
                ...
                <div className="flex justify-between items-center bg-card p-4 rounded-2xl shadow-sm border border-border">
                    ...
                    <div className="text-center">
                        <p className="text-xs text-muted-foreground uppercase tracking-wider font-bold">Today's Menu</p>
                    ...
                    {Object.entries(currentDay.meals).map(([type, meal]) => (
                        <div key={type} className="bg-card p-5 rounded-2xl border border-border shadow-sm relative group overflow-hidden">
                            ...
                            <div className="flex justify-between items-start mb-2 relative z-10">
                                <span className="text-xs uppercase font-bold text-primary tracking-wider">{type}</span>
                                <button onClick={() => handleSwap(type)} className="text-muted-foreground hover:text-primary transition-colors">
                                    <RefreshCw size={16} />
                                </button>
                            </div>
                ...
                <div className="bg-accent/30 p-4 rounded-xl border border-primary/20 mt-8">
                    <h4 className="font-bold text-sm mb-2 flex items-center gap-2">
                        <MessageCircle size={16} className="text-primary" /> Request Changes
                    </h4>
                    <p className="text-xs text-muted-foreground mb-3">Does something not fit? Let your nutritionist know.</p>
                    <button
